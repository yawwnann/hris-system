...
<template>
  <Card class="bg-white dark:bg-zinc-950 border-gray-200 dark:border-zinc-800 shadow-sm overflow-hidden">
    ...
    <CardContent class="p-4 space-y-5">
      ...
      <!-- Away Hari Ini -->
      <div>
        <h4 class="text-xs font-semibold text-gray-500 dark:text-zinc-400 uppercase tracking-wider mb-3">Sedang Cuti Hari Ini ({{ awayToday.length }})</h4>
        <div v-if="awayToday.length === 0" class="text-xs text-gray-400 italic">Tidak ada yang cuti hari ini.</div>
        <div class="space-y-3 max-h-[140px] overflow-y-auto pr-2 custom-scrollbar">
          <div v-for="person in awayToday" :key="person.id" class="flex items-center space-x-3 group">
            ...
            <div class="flex-1 min-w-0">
              <p class="text-sm font-medium text-gray-900 dark:text-gray-100 truncate group-hover:text-orange-600 transition-colors">{{ person.user }}</p>
